...
public class ProductPage {
	...
	public void click_product_size() {
		...
		FluentWait<WebDriver> wait1 = new FluentWait<WebDriver>(driver).withTimeout(30, TimeUnit.SECONDS)
	...
	public void events_to_add_product_to_shopping_cart(String product_selected, List<String> product_name_list,
			List<String> product_price_list) {
		this.click_product_name(product_selected);
		this.wait_for_page_to_load();
		String product_name = this.get_product_name();
		product_name_list.add(product_name);
		String price_value = this.get_product_price();
		product_price_list.add(price_value);
		this.click_product_size();
		this.click_product_color();
		this.click_add_to_cart();
		this.wait_for_success_message();
